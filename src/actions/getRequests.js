...
export const getMedicineByPharmacy = (pharmacy_id) => (dispatch) => {
    const requestResult = axios.post(API_URL + `firstRequestPartOne`, {pharmacy_id}, {headers: authHeader()})
...
export const getMedicineByArea = () => {
    return async (dispatch) => {
        const requestResult = await axios.get(API_URL + `firstRequestPartTwo`, {headers: authHeader()});
...
export const getSecondRequestFirstPart = (currentPage, perPage) => {
    return async (dispatch) => {
        const requestResult = await axios.get(API_URL + `secondRequestFirstPart?page=${currentPage}&limit=${perPage}`, {headers: authHeader()});
...
export const getSecondRequestSecondPart = () => {
    return async (dispatch) => {
        const requestResult = await axios.get(API_URL + `secondRequestSecondPart`, {headers: authHeader()});
...
export const getThirdRequest = (searchQuery, currentPage, perPage) => {
    ...
    return async (dispatch) => {
        ...
        const requestResult = await axios.get(API_URL + `thirdRequest?searchQuery=${searchQuery}&page=${currentPage}&limit=${perPage}`, {headers: authHeader()});
...
export const getDateFistRequest = (date, currentPage, perPage) => {
    return async (dispatch) => {
        ...
        const requestResult = await axios.post(API_URL + `dateFirstRequest?page=${currentPage}&limit=${perPage}`, {date}, {headers: authHeader()});
...
export const getDateSecondRequest = (date, currentPage, perPage) => {
    return async (dispatch) => {
        ...
        const requestResult = await axios.post(API_URL + `dateSecondRequest?page=${currentPage}&limit=${perPage}`, {date}, {headers: authHeader()});
        if (currentPage > requestResult.data.totalPages && requestResult.data.totalPages !== 0) {
