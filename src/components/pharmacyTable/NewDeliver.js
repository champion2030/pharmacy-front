...
import {getEmployeesForCurrentPharmacy} from "../../actions/getEmployee";
...
const NewDeliver = (props) => {
    ...
    const allEmployeesForCurrentPharmacy = useSelector(state => state.employeeReducer.allEmployeesForCurrentPharmacy)
    const [successful, setSuccessful] = useState(false);
    const {message} = useSelector(state => state.message);

    useEffect(() => {
        dispatch(getAllMedicines())
        dispatch(getEmployeesForCurrentPharmacy(id))
        dispatch(getReasons())
                ...
                <Grid container align="center" justify="center" alignItems="center">
                    <Grid item xs={3}>
                        <Autocomplete
                            ...
                            onChange={(event, newValue) => {setMedicineId(newValue.id)}}
                            renderInput={(params) =>
                                <TextField
                                    {...params}
                    ...
                    <Grid item xs={3}>
                        <Autocomplete
                            id="combo-box-demo2"
                            options={allEmployeesForCurrentPharmacy}
                            disableClearable
                            getOptionLabel={(option) => option.full_name}
                            style={{width: 300, marginBottom: 20}}
                            onChange={(event, newValue) => {setEmployeeId(newValue.id)}}
                            renderInput={(params) =>
                                <TextField
                                    {...params}
                                    label={"Сотрудник"}
                    ...
                    <Grid item xs={3}>
                        <Autocomplete
                            ...
                            onChange={(event, newValue) => {setCauseId(newValue.id)}}
                            renderInput={(params) =>
                                <TextField
                                    {...params}
